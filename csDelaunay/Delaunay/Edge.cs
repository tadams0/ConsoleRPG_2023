...
namespace csDelaunay {
	...
	public class Edge<T> {
		...
		#region Pool
		private static Queue<Edge<T>> pool = new Queue<Edge<T>>();
		 ...
		 * @return
		 */
		public static Edge<T> CreateBisectingEdge(Site<T> s0, Site<T> s1) {
			float dx, dy;
			float absdx, absdy;
			float a, b, c;
		...
		public static List<Edge<T>> SelectEdgesForSitePoint(Vector2f coord, List<Edge<T>> edgesToTest) {
		...
		public static readonly Edge<T> DELETED = new Edge<T>();
		...
		public static int CompareSitesDistances_MAX(Edge<T> edge0, Edge<T> edge1) {
		...
		public static int CompareSitesDistances(Edge<T> edge0, Edge<T> edge1) {
		...
		private LRCollection<Site<T>> sites;
		public Site<T> LeftSite {get{return sites[LR.LEFT];} set{sites[LR.LEFT]=value;}}
		public Site<T> RightSite {get{return sites[LR.RIGHT];} set{sites[LR.RIGHT]=value;}}

		public Site<T> Site(LR leftRight) {
