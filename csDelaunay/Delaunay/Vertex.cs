...
namespace csDelaunay {
	...
	public class Vertex : ICoord {
		 ...
		 * 
		 */
		public static Vertex Intersect<T>(Halfedge<T> halfedge0, Halfedge<T> halfedge1) {
			Edge<T> edge, edge0, edge1;
			Halfedge<T> halfedge;
			...
			if (Voronoi<T>.CompareByYThenX(edge0.RightSite, edge1.RightSite) < 0) {
				halfedge = halfedge0;
				edge = edge0;
			} else {
				halfedge = halfedge1;
