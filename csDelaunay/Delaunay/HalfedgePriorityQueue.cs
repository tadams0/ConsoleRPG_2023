...
namespace csDelaunay {
	...
	// Also know as heap
	public class HalfedgePriorityQueue<T>
    {
		...
		public void Init() {
			...
			for (int i = 0; i < hashSize; i++) {
				hash[i] = Halfedge<T>.CreateDummy();
				hash[i].nextInPriorityQueue = null;
			}
		}

		public void Insert(Halfedge<T> halfedge) {
			Halfedge<T> previous, next;
		...
		private int Bucket(Halfedge<T> halfedge) {
		...
		public Vector2f Min() {
			AdjustMinBucket();
			Halfedge<T> answer = hash[minBucked].nextInPriorityQueue;
